...
* Движок распознавания: `faster-whisper` (CPU/CTranslate2). Модель `Systran/faster-whisper-base`
  скачивается только на машине сборки и добавляется в `assets/models/faster-whisper-base`.
...
скачивает модель `Systran/faster-whisper-base`, собирает PyInstaller one-folder и
...
`assets/models/faster-whisper-base`; с `-Offline` сборка не обращается к сети.
PyInstaller 6 помещает модель в `LocalDictationTray\_internal\models\faster-whisper-base`;
...
* PyInstaller 6 puts bundled data below `_internal`. The package check therefore
  accepts `_internal/models/faster-whisper-base` (and the legacy root layout), while
